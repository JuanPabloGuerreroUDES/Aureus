...
<%-- ── FILTROS (FIX: input type=month con value preseleccionado + cuentaId visible) ── --%>
<div class="card border-0 shadow-sm mb-3" style="border-radius:12px">
    <div class="card-body py-3">
        <form method="get" action="/transacciones" class="d-flex gap-2 align-items-center flex-wrap">
            ...
            <%-- FIX: preseleccionar el mes actual (periodoActual viene del controller) --%>
            <input type="month" name="periodo" class="form-control form-control-sm" style="width:auto"
                   value="${periodoActual}">
            <button type="submit" class="btn btn-sm btn-outline-secondary">
                <i class="bi bi-funnel"></i> Filtrar
            </button>
            <c:if test="${not empty cuentaSeleccionada}">
                <a href="/transacciones?cuentaId=${cuentaSeleccionada}" class="btn btn-sm btn-link text-muted p-0">
                ...
                <c:choose>
                    <c:when test="${not empty transacciones}">
                        <c:forEach var="t" items="${transacciones}">
                            <tr>
                                <td class="ps-3" style="color:#6E7080;font-size:.8rem">${t.date}</td>
                                <td>
                                ...
                                <td class="text-center pe-3">
                                    <form action="/transacciones/${t.id}/eliminar" method="post"
                                          class="d-inline"
                            ...
                            <td colspan="6" class="text-center text-muted py-5">
                                ...
                                <c:choose>
                                    <c:when test="${empty cuentaSeleccionada}">
                                        Selecciona una cuenta arriba para ver sus transacciones.
                                    </c:when>
                                    <c:otherwise>
                                        No hay transacciones en este período.
                                        <a href="#" data-bs-toggle="modal" data-bs-target="#modalNueva"
                            ...
                            <select name="accountId" class="form-select form-select-sm" required>
                                ...
                                <c:forEach var="cuenta" items="${cuentas}">
                                    <option value="${cuenta.id}"
                                            ${cuenta.id == cuentaSeleccionada ? 'selected' : ''}>
...
<%-- Preseleccionar fecha de hoy en el modal --%>
<script>
document.addEventListener('DOMContentLoaded', function() {
    const fechaInput = document.getElementById('fechaTx');
